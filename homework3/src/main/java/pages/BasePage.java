...
public class BasePage {
    ...
    Stack<Product> bucket = new Stack<>();
    ...
    public CartPage openCartPage() {
        waitUtilElementToBeClickable(purchasesSum).click();

        return pageManager.getCartPage().checkOpenPage();
    }

    public void checkPurchasesSum(){
        int sum = 0;
        for (int i = 0; i < dataManager.getProductList().size(); i++) {
            sum += Utils.getProductTotalPrice(dataManager.getProductList().get(i));
        }

        Assert.assertEquals("Сумма покупок не соответсвует действительности!", Integer.valueOf(sum), Utils.convertPrice(purchasesSum.getText()) );
    }
     ...
     * @see WebDriverWait
     */
    protected WebDriverWait wait = new WebDriverWait(driverManager.getDriver(), 35, 3500);
    ...
    public void checkAddProductToCart() {
        waitTextToBePresent(productCounter, (dataManager.getProductList().size() + 1) + "");
    }
